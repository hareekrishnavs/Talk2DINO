"""Checkpoint schema/invariant validation for the ADE20K C150 matched
E3-vs-k11-vs-k12 evaluator. Sole authority for the checkpoint's
schema/type/binding/completeness checks -- never reimplemented at any
call site. A VOC2012 or COCO-Object checkpoint (wrong dataset binding)
must be rejected."""
...
def _require_int(value: Any, label: str, *, minimum: int | None = None, maximum: int | None = None) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise Ade20kMatchedEvaluatorIdentityError(f"{label} must be an exact non-boolean integer")
    if minimum is not None and value < minimum:
        raise Ade20kMatchedEvaluatorIdentityError(f"{label} must be at least {minimum}")
    if maximum is not None and value > maximum:
        raise Ade20kMatchedEvaluatorIdentityError(f"{label} must be at most {maximum}")
    return value
...
def _require_string_list(value: Any, label: str) -> list[str]:
    if type(value) is not list or any(type(item) is not str for item in value):
        raise Ade20kMatchedEvaluatorIdentityError(f"{label} must be a list of exact strings")
    return value
...
def validate_checkpoint_structure(
    ...
    if _require_exact_string(checkpoint["schema"], "checkpoint.schema") != identity["checkpoint"]["schema_name"]:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.schema mismatch")
    if _require_exact_string(checkpoint["identity"], "checkpoint.identity") != identity["identity"]["name"]:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.identity mismatch")
    if _require_sha256(checkpoint["identity_sha256"], "checkpoint.identity_sha256") != identity_sha256:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.identity_sha256 does not match the loaded identity file")
    if checkpoint["matched_identity_sha256"] != identity["parent_identities"]["matched_identity_sha256"]:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.matched_identity_sha256 disagrees with the identity's parent")
    if checkpoint["ade20k_source_identity_sha256"] != identity["parent_identities"]["ade20k_source_identity_sha256"]:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.ade20k_source_identity_sha256 disagrees with the identity's parent")
    _require_sha256(checkpoint["ade20k_source_manifest_sha256"], "checkpoint.ade20k_source_manifest_sha256")
    ...
    if recorded_run_mode not in ("pilot20", "pilot100", "full"):
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.run_mode must be one of pilot20, pilot100, full")
    if run_mode is not None and recorded_run_mode != run_mode:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.run_mode disagrees with --run-mode")
    ...
    if recorded_class_count != identity["dataset"]["class_count"]:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.class_count disagrees with identity.dataset.class_count")
    if class_count is not None and recorded_class_count != class_count:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.class_count disagrees with the live inference class count")
    ...
    if len(set(completed)) != len(completed):
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.completed_image_ids contains a duplicate image ID")
    ...
    if images_completed_count != len(completed):
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.images_completed_count disagrees with len(completed_image_ids)")
    ...
    if complete and next_index != expected_image_count:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.complete is true but next_dataset_index does not equal the expected image count")
    ...
    if images_completed_count > 0 and windows_processed_total < images_completed_count:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.windows_processed_total must be at least images_completed_count")
...
def validate_checkpoint_against_canonical_order(
    ...
    if checkpoint["image_order_digest"] != image_order_digest:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint.image_order_digest disagrees with the freshly-resolved dataset order")
...
def resume_dataset_index(checkpoint: Mapping[str, Any]) -> int:
    if checkpoint["complete"] is True:
        raise Ade20kMatchedEvaluatorIdentityError("checkpoint is already complete; refusing to resume it as though incomplete")
