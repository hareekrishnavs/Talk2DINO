...
This cleanup contains only the baseline Talk2DINO implementation and its optional PAMR refinement. Model loading is local-only: set `TALK2DINO_WEIGHT_DIR` to the directory containing checkpoints (the default is `weights/`). Expected filenames include `ViT-B-16.pt`, `dinov2_vitb14_reg4_pretrain.pth`, `dinov2_vitl14_reg4_pretrain.pth`, `vit_base_patch16_dinov3.lvd1689m.pth`, and `vit_large_patch16_dinov3.lvd1689m.pth`.

The `model.weight_dir`, `model.clip_model_path`, and `model.backbone_weights` configuration fields override the directory and individual checkpoint paths. Relative paths are resolved below `weight_dir`.

Run the single-scale COCO-Stuff baseline evaluation (PAMR disabled) with:

```bash
python -m torch.distributed.run src/open_vocabulary_segmentation/main.py --eval \
  --eval_cfg src/open_vocabulary_segmentation/configs/stuff/dinotext_stuff_vitb_mlp_infonce.yml \
